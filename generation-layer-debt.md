...
| `mvc-parametric-curves` | math/plot emission: helix should render as `parametric3d` | Generation-layer: Agent 3 describes the helix `r(t)=<cos t, sin t, t>` but emits only a `surface3d` half-cylinder, so the live prose was patched to describe the cylinder accurately without changing the plot block. | Text-only production patch on block `ea4fe227-2d63-420d-8ddc-950969cd720c`; plot block `0ecaef65-9125-47a5-ac6d-5b4cf33aaf9a` remains unchanged. |
